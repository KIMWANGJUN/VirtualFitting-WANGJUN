...
const VirtualFittingMainPage = () => {
  ...
  const handleProcessClick = (process) => {
    if (process.status === 'COMPLETED') {
      // 새 탭으로 결과 선택 페이지 열기
      const url = `/virtual-fitting-result/${process.process_id}`
  ...
  // 이미지 미리보기 함수
  const handlePreviewImage = (imageUrl, fittingId) => {
    // 새 탭에서 미리보기 페이지로 이동해 입력/결과 함께 표시
    const url = `/virtual-fitting-result/${fittingId || ''}`
    window.open(url, '_blank', 'noopener,noreferrer')
  }
                ...
                <div key={result.fitting_id} className={styles.savedCard}>
                  <div className={styles.savedImageWrapper}>
                    ...
                    <div className={styles.savedImageOverlay}>
                      <button 
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          window.open(`/virtual-fitting-result/saved/${result.fitting_id}`, '_blank', 'noopener,noreferrer')
                        }}
                        title="미리보기"
                      >
                      ...
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          try {
                            await deleteFittingResult(result.fitting_id)
                            await loadData(true)
                          } catch (err) {
                  ...
                  <div className={styles.savedCardContent}>
                    <h3 className={styles.savedTitle}>
                      {result.title || `가상 피팅 결과 ${result.fitting_id}`}
                    </h3>
