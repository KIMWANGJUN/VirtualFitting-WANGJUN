...
// 프로세스 입력 이미지 조회 URL
export const getProcessInputUrl = (processId, kind /* 'model'|'cloth' */) => {
  return `${API_BASE_URL}/api/virtual-fitting-redis/inputs/${processId}/${kind}`
}

// 저장 결과 입력 이미지 조회 URL
export const getResultInputUrl = (fittingId, kind /* 'model'|'cloth' */) => {
  return `${API_BASE_URL}/api/virtual-fitting-redis/result-inputs/${fittingId}/${kind}`
}

// 저장된 결과 삭제
export const deleteFittingResult = async (fittingId) => {
  const res = await fetch(`${API_BASE_URL}/api/virtual-fitting-redis/result/${fittingId}`, {
    method: 'DELETE',
    credentials: 'include',
  })
  if (!res.ok) {
...
// 저장 결과 재생성(재선택용 새 프로세스 생성)
export const respinSavedFitting = async (fittingId, {
  category = 0,
  modelType = 'dc',
  scale = 2.0,
  samples = 4,
  ...
  const res = await fetch(`${API_BASE_URL}/api/virtual-fitting-redis/respin/${fittingId}`, {
    method: 'POST',
    credentials: 'include',
    body: form,
  })
  ...
  return res.json() // { success, process_id }
}

// 저장된 가상 피팅 목록 조회 (미리보기 화면용)
export const getSavedFittingsList = async () => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/virtual-fitting-redis/saved-list`, {
      credentials: "include",
    })
    if (!response.ok) {
      throw new Error("저장된 가상 피팅 목록 조회에 실패했습니다.")
    }
    return await response.json()
  } catch (error) {
    console.error("저장된 가상 피팅 목록 조회 오류:", error)
    throw error
  }
}

// 저장된 가상 피팅의 원본 선택지 조회
export const getSavedFittingOriginalOptions = async (fittingId) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/virtual-fitting-redis/saved-original-options/${fittingId}`, {
      credentials: "include",
    })
    if (!response.ok) {
      throw new Error("저장된 가상 피팅 원본 선택지 조회에 실패했습니다.")
    }
    return await response.json()
  } catch (error) {
    console.error("저장된 가상 피팅 원본 선택지 조회 오류:", error)
    throw error
  }
}

// 저장된 가상 피팅 선택지 이미지 URL 생성
export const getSavedFittingOptionImageUrl = (fittingId, optionIndex) => {
  return `${API_BASE_URL}/api/virtual-fitting-redis/saved-option/${fittingId}/${optionIndex}`
}

// 저장된 가상 피팅 결과의 원본 선택지로 재선택
export const reselectFittingOption = async (fittingId, optionIndex) => {
  try {
    const formData = new FormData()
    formData.append('option_index', optionIndex)
    
    const response = await fetch(`${API_BASE_URL}/api/virtual-fitting-redis/reselect-option/${fittingId}`, {
      method: "POST",
      credentials: "include",
      body: formData
    })
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.detail || "재선택에 실패했습니다.")
    }
    
    return await response.json()
  } catch (error) {
    console.error("재선택 오류:", error)
    throw error
  }
}
